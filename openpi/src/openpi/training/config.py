...
class LeRoboAutoBioDataConfig(DataConfigFactory):
    """Data config for AutoBio (https://arxiv.org/abs/2505.14030) tasks."""

    real_action_dim: int = 7

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        repack_transform = _transforms.Group(inputs=[autobio_policy.AutoBioRepack()])
...
_CONFIGS = [
    ...
    # AutoBio (https://arxiv.org/abs/2505.14030) configs, fine-tuned from pi0.5 with LoRA.
    TrainConfig(
        name="pi05_thermal_cycler_close",
        model=pi0_config.Pi0Config(
