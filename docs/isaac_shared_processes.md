...
后续进展见 [完整双容器移液与共享任务判定](isaac_pipette_transfer.md)。本文的 190 回合、61 项单元测试及旧任务谓词结果保留为当时冻结版本的历史记录。提交署名改写后的等价代码见 [版本映射](validation/isaac_commit_identity_mapping.json)，原始证据哈希保持原值。
...
下图分别由两个引擎恢复实际记录的关节状态与液面日志后渲染。Isaac 关键帧回放一次出图调用等待 8 个渲染帧后读取 RGB，运行器记录的物理时间和步数保持不变。旧报告没有独立计数 Kit 内部初始化，不能证明初始化没有物理步骤；[新离心关键帧报告](validation/isaac_cycle_isaac_keyframes.json) 实际记录内部初始化 2 步，恢复状态后渲染 0 个物理事件。相同阶段的记录时间可能不同；图像不是新闭环回合，也未取得像素等价性认证。[图片、轨迹、液面和源快照哈希](validation/isaac_shared_keyframes.json) 随分支保存，局域网图库另提供前视和腕部相机共 24 张图。
