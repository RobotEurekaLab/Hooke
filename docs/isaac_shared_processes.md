...
2026-09-16，固定种子 0–9。运行代码冻结于 `79bc811`；后续 `b8bff48` 仅补充错误路径处理，证据核对工具另行加入。58 项单元测试通过。
...
| 移液 | 10/10 | 13.786–14.800 | 10/10 |

涡旋保留 30 秒时限，终态归位误差为 0.489–0.969 mm；10 个回合未检测到两臂接触。移液吸头终量为 200 µL，源液面体积与记账一致。涡旋和移液各 10 个、持续 30 秒的空动作对照均未误通过。
...
体积验收要求吸头终量距标称容量不超过 5%、最大总体积误差及源液面体积误差不超过 `1e-12 m³`、储量满足容量边界、没有液体排入环境。这些是理想模型的记账条件，不能作为真实移液器精度指标。模型未模拟压力、气体压缩性、气泡、滴落、残液、倾倒溢出或流量标定，液量变化也未反馈到容器质量与惯性。
