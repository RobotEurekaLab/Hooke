...
class InsertCentrifuge5430(Task):
    ...
    @classmethod
    def for_variant(cls, scene_path, task_name: str, slot_window: tuple = (-4, 9)):
        """Builds a Task/Expert pair identical to this one but pointed at a
        different scene file (e.g. a rotor-capacity variant generated by
        archetypes/rotor_variants.py), optionally with its own reachable-slot
        window. Nothing else about the task changes."""
        variant_task = type(
            f"{cls.__name__}_{task_name}", (cls,),
            dict(default_scene=scene_path, default_task=task_name, slot_window=slot_window),
        )
        variant_expert = type(
            f"{cls.Expert.__name__}_{task_name}", (variant_task, cls.Expert),
            {},
        )
        variant_task.Expert = variant_expert
        return variant_task, variant_expert
    ...
    # Reachable/visible slot-index window for the scripted expert, expressed
    # relative to slot 0. Tuned against the original 30-slot rotor; a rotor
    # capacity variant (see archetypes/rotor_variants.py) may need its own
    # window if that assumption doesn't hold at a different slot spacing --
    # check the per-variant success rate rather than assuming it transfers.
    slot_window: tuple = (-4, 9)
